...
import TrialStakeholders, {
  stakeholdersFromApi,
  stakeholdersToApi,
} from "../../components/items/TrialStakeholders/TrialStakeholders";
import {
  CATEGORIES,
  DEVELOPMENT_STATUS,
  TOT_STATUS,
...
function Checkbox({ checked, onChange }) {
  return (
    <div
      className={`eif__check${checked ? " eif__check--on" : ""}`}
      onClick={onChange}>
      {checked && (
        <svg
          viewBox="0 0 12 12"
...
function EditVariantFields({ item, itemName, variantId, onCancel, onSuccess }) {
  ...
  // Tracks what's actually persisted on the server (as opposed to an
  // unsaved local pick still sitting in `imageFile`/`imagePreview`), so
  // "Remove" can tell "discard my pending pick" apart from "actually
  // delete the image" — see the onClick below.
  ...
  const [docMenuOpenFor, setDocMenuOpenFor] = useState(null);
  ...
  useEffect(() => {
    ...
    setServerImageUrl(existingImage);
    ...
    reset({
      ...
      productDevCompletionDate: toInputDate(item.productDevCompletionDate),
    ...
    setTotCerts(
      Object.keys(TOT_DOCUMENTS).reduce(
        (acc, k) => ({
          ...acc,
          [k]: (item.totDocumentsFiled || []).includes(k),
        }),
        {},
      ),
    );
    ...
    setIprData({
      patent: {
        ...
        inventor: item.iprDetail?.patentInventor || "",
        filingNo: item.iprDetail?.patentFilingNo || "",
        filingDate: item.iprDetail?.patentFilingDate || "",
        grantNo: item.iprDetail?.patentGrantNo || "",
        grantDate: item.iprDetail?.patentGrantDate || "",
      },
      trademark: {
        filed: item.iprDetail?.trademarkFiled || false,
        granted: item.iprDetail?.trademarkGranted || false,
        inventor: item.iprDetail?.trademarkInventor || "",
        filingNo: item.iprDetail?.trademarkFilingNo || "",
        filingDate: item.iprDetail?.trademarkFilingDate || "",
        grantNo: item.iprDetail?.trademarkGrantNo || "",
        grantDate: item.iprDetail?.trademarkGrantDate || "",
      },
      design: {
        filed: item.iprDetail?.designFiled || false,
        granted: item.iprDetail?.designGranted || false,
        inventor: item.iprDetail?.designInventor || "",
        filingNo: item.iprDetail?.designFilingNo || "",
        filingDate: item.iprDetail?.designFilingDate || "",
        grantNo: item.iprDetail?.designGrantNo || "",
        grantDate: item.iprDetail?.designGrantDate || "",
      },
      copyright: {
        filed: item.iprDetail?.copyrightFiled || false,
        granted: item.iprDetail?.copyrightGranted || false,
        inventor: item.iprDetail?.copyrightInventor || "",
        filingNo: item.iprDetail?.copyrightFilingNo || "",
        filingDate: item.iprDetail?.copyrightFilingDate || "",
        grantNo: item.iprDetail?.copyrightGrantNo || "",
        grantDate: item.iprDetail?.copyrightGrantDate || "",
      },
    });

    setStakeholders(stakeholdersFromApi(item.trialStakeholders));

    setProcurements(
      item.procurementDetails?.map((p) => ({
        id: p.id || uid(),
        agency: p.procurementAgency || "",
  ...
  const onSubmit = async (data) => {
    ...
    try {
      // FIX: this used to call updateItemAsync, which PUTs the *base item*
      // endpoint — so saving a variant's edits silently overwrote the
      // parent item's own data (or 404'd, since `item.id` here is the
      // parent item's id, not the variant's) instead of touching this
      // variant's independent record at all. updateVariantAsync targets
      // PUT /items/{id}/variants/{variantId}, the actual variant endpoint.
      // `item.version` is the variant's own optimistic-lock version (see
      // the merge in the EditVariantForm wrapper below), not the item's.
      await dispatch(
        updateVariantAsync({ id: item.id, variantId, version: item.version, ...payload })
      ).unwrap();
      if (imageFile) {
      ...
      // Errors are already surfaced via toast inside updateVariantAsync/uploadImageAsync.
    }
  };

  // All react-hook-form `required` fields live on Step 1 (Basic Information).
  // The wizard lets you jump between steps freely, so saving from a later
  // step while step 1 is invalid used to fail silently. Send the user back
  // to the problem step and say why.
  const onInvalid = () => {
    setStep(1);
  ...
  const handleDocFileChosen = async (e) => {
    ...
    <>
        ...
        <div className="eif__stepper-meta">
          <div className="eif__stepper-meta-left">
            <span className="eif__eyebrow">
              Editing Item &middot; Step {step} of {STEPS.length}
            </span>
            ...
            return (
              <div
                ...
                {hasError ? (
                  ...
                  <span className="eif__tab-dot eif__tab-dot--done">
                    {Icons.check}
                  </span>
                ) : (
                  <span
                    className={`eif__tab-dot${isActive ? " eif__tab-dot--active" : ""}`}>
                    {s.id}
                  </span>
                )}
      ...
      <div className="eif__card" data-accent={STEP_COLORS[cur.icon]}>
        ...
        <div className="eif__card-head">
          <span
            className={`eif__card-icon eif__card-icon--${STEP_COLORS[cur.icon]}`}>
            {Icons[cur.icon]}
          </span>
          ...
          <>
            <div className="eif__s1-layout">
              <div className="eif__img-col">
                <div
                  className={`eif__img-box${imagePreview ? " eif__img-box--filled" : ""}`}
                  onClick={() => fileInputRef.current?.click()}>
                  {imagePreview ? (
                    <img
                      src={imagePreview}
                      alt="preview"
                      className="eif__img-preview"
                    />
                  ) : (
                    <>
                      <span className="eif__img-icon">{Icons.upload}</span>
                      <span className="eif__img-label">Upload Image</span>
                      <span className="eif__img-hint">
                        Click to browse
                        <br />
                        JPG, PNG up to 5MB
                      </span>
                ...
                {imagePreview && (
                  <button
                    type="button"
                    className="eif__img-remove"
                    disabled={removingImage}
                    onClick={async () => {
                      if (imageFile) {
                        // Just discard the not-yet-saved local pick and fall back to
                        // whatever image is actually still on the server (if any).
                        setImageFile(null);
                      ...
                      const warning = hasVariants
                        ? "This image is shared by the item and all its variants. Removing it will remove it everywhere, not just here. Continue?"
                        : "Remove this item's image? This can't be undone.";
                      if (!window.confirm(warning)) return;
                      setRemovingImage(true);
                      const res = await dispatch(deleteImageAsync(item.id));
            ...
            {/* ── Variants note ── */}
            <div className="eif__variant-note">
              <span className="eif__variant-note-icon">{Icons.file}</span>
              <div>
                <strong>Variants are managed separately.</strong>
                <p>
                  Use <em>“Manage Variants”</em> on the item's detail page to
                  add, edit or remove variants — each one has its own
                  independent Basic Info, ToT, IPR, Trial Stakeholders,
            ...
            <div className="form-group">
              ...
              <div className="eif__radio-row">
                {Object.values(TOT_STATUS).map((opt) => (
                  <label
                    key={opt}
                    className="eif__radio-item"
                    onClick={() => setTotStatus(opt)}>
                    <span
                      className={`eif__radio${totStatus === opt ? " eif__radio--on" : ""}`}>
                      ...
                      totPartners.map((p) => (
                        <tr key={p.id}>
                          ...
                          <td style={{ display: "flex", gap: 6 }}>
                            <button
                              type="button"
                              className="eif__action-btn eif__action-btn--edit"
        ...
        {step === 3 && (
          <div className="eif__step-body">
            <div className="eif__ipr-wrap">
              {[
                {
                  key: "patent",
                  title: "Patent",
                  fLbl: "Patent Filing No",
                  gLbl: "Patent Grant No / Granting No",
                },
                {
                  key: "trademark",
                  title: "Trademark",
                ...
                <div key={key} className="eif__ipr-block">
                  <div className="eif__ipr-title">
                    <span className="eif__ipr-title-icon">{Icons.ipr}</span>
                    {title}
                  </div>

                  <div className="form-group eif__ipr-inventor">
                    <label className="form-label">Inventor</label>
                    <input
                      className="form-control"
                  ...
                  <div className="eif__ipr-grid">
                    ...
                    <div className="eif__ipr-check-row">
                      ...
                      />
                      <span
                        className={`eif__ipr-lbl${!iprData[key].filed ? " eif__ipr-lbl--disabled" : ""}`}>
                        Granted{" "}
                        {!iprData[key].filed && (
                          <span className="eif__ipr-hint">(File first)</span>
                        )}
                      </span>
                    </div>
                    <div className="form-group" style={{ margin: 0 }}>
          ...
          <div className="eif__step-body">
            ...
            <div className="eif__doc-list">
              {[...DOCUMENTATION_ITEMS, ...customDocuments].map((d) => {
                const attached = item?.uploadedDocuments?.find(
                  (f) => (f.docName || f.originalFileName) === d,
                );
                return (
                  <div
                    key={d}
                    className="eif__doc-row"
                    onClick={() => toggleDoc(d)}>
                    ...
                    {attached ? (
                      <div
                        className="eif__doc-menu-wrap"
                        onClick={(e) => e.stopPropagation()}>
                        <button
                          type="button"
                          className="eif__doc-menu-btn"
                          onClick={() =>
                            setDocMenuOpenFor(docMenuOpenFor === d ? null : d)
                          }>
                          {Icons.moreVert}
                        </button>
                        {docMenuOpenFor === d && (
                          <>
                            <div
                              className="eif__doc-menu-backdrop"
                              onClick={() => setDocMenuOpenFor(null)}
                            />
                            <div className="eif__doc-menu">
                              <a
                                className="eif__doc-menu-item"
                              ...
                              <button
                                type="button"
                                className="eif__doc-menu-item eif__doc-menu-item--danger"
                                disabled={docDeletingId === attached.id}
                                onClick={() => {
                                  setDocMenuOpenFor(null);
                                  handleDeleteDoc(attached.id);
                                }}>
                                {Icons.trash} Delete
                              </button>
                            </div>
                      ...
                      <button
                        type="button"
                        className="eif__doc-upload-btn"
                        title={
                          checkedDocs.has(d)
                            ? `Upload file for ${d}`
                            : "Check this document first to enable upload"
                        }
                        disabled={docUploading || !checkedDocs.has(d)}
                        onClick={(e) => {
                          e.stopPropagation();
        ...
        <div className="eif__footer">
          <div>
            {step > 1 && (
              <button
                type="button"
                className="eif__nav-btn eif__nav-btn--prev"
                onClick={goPrev}>
          ...
          <div className="eif__footer-right">
            <button
              type="button"
              className="eif__nav-btn eif__nav-btn--cancel"
              onClick={onCancel}>
              {Icons.close} Cancel
            </button>
            {step < STEPS.length ? (
              <button
                type="button"
                className="eif__nav-btn eif__nav-btn--next"
                onClick={goNext}>
              ...
              <button
                type="button"
                className="eif__nav-btn eif__nav-btn--save"
                disabled={submitting}
                onClick={handleSubmit(onSubmit, onInvalid)}>
                {submitting ? <span className="eif__spin" /> : Icons.check}
                {submitting ? "Saving…" : "Update Item"}
...
/*
 * ── PAGE WRAPPER ──
 * This used to be the file's default export, rendered by the router as
 * <EditVariantForm /> at /items/:id/variants/:variantId/edit with no props
 * at all. EditVariantFields above only ever does anything once it receives
 * an `item` prop — so every field silently stayed at its empty default
 * (0% complete, "No stakeholders added yet", etc.) no matter which variant
 * you opened, and Cancel/Save had nothing to call back into either. This
 * wrapper is what EditItem.jsx already does for the base item: read the
 * ids from the URL, fetch the item, and hand down real data.
 */
export default function EditVariantForm() {
  ...
  if (detailLoading || !selectedItem) {
    return <Loader variant="page" text="Loading variant..." />;
  }

  const variant = selectedItem.variants?.find((v) => String(v.id) === String(variantId));
  ...
  const back = () => navigate(`/items/${id}?variant=${variantId}`);
  ...
  return (
    <div className="edit-item-page animate-fade-in-up">
      <nav className="edit-item-page__breadcrumb">
        ...
        <span className="edit-item-page__crumb-link" onClick={back}>
          {selectedItem.name}
        </span>
        <Icon name="forward" size={12} strokeWidth={2.5} className="edit-item-page__crumb-arrow" />
        <span className="edit-item-page__crumb-cur">Edit — {variant.name}</span>
      </nav>
