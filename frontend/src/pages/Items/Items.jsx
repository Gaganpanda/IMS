...
export default function Items() {
  ...
  const dashStatsLoading = useSelector((s) => s.dashboard.loading);
  ...
  // Stat cards (Developed / In Progress / Trials / IPR Filed) read from
  // dashboard.stats in Redux. Previously only the Dashboard page fetched this,
  // so landing on /items directly (fresh login, refresh, deep link) showed
  // "—" on every card until the user visited /dashboard at least once.
  // Fetch it here too, guarded so we don't refire while already loading or
  // once it's already populated in this session.
  useEffect(() => {
    if (dashStats == null && !dashStatsLoading) {
      dispatch(fetchDashboardStatsAsync());
    }
  }, [dispatch, dashStats, dashStatsLoading]);
        ...
        <div className="items-page__tabs">
          ...
          <div className="items-page__tabs-right">
            ...
            <select className="items-page__sort" value={sortVal}
              onChange={(e) => { setSortVal(e.target.value); dispatch(setPage(0)); }}>
              {SORT_OPTIONS.map((o) => (
