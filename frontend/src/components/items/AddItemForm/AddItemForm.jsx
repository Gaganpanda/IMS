...
import { createItemAsync, uploadImageAsync, uploadDocumentAsync } from "../../../redux/slices/itemSlice";
    ...
    try {
      ...
      if (created?.id && pendingFiles.length) {
        for (const f of pendingFiles) {
          await dispatch(uploadDocumentAsync({ id: created.id, name: f.name, file: f.file }));
        }
      }
      onSuccess?.();
    } catch (_) {}
    ...
    <>
      ...
      <div className="aif__card">
        ...
        <div className="aif__card-head">
          <span className={`aif__card-icon aif__card-icon--${STEP_COLORS[cur.icon]}`}>
            {Icons[cur.icon]}
          </span>
          ...
          <>
            ...
            <div className="aif__variant-note">
              ...
              <div>
                ...
                <p>Save this item first, then open it and use <em>“Manage Variants”</em> to add
                  variants — each one gets its own independent Basic Info, ToT, IPR, Trial
                  Stakeholders, Documentation and Procurement details.</p>
              </div>
            </div>
          </>
        )}
          ...
          <div className="aif__step-body">
            ...
            <div>
              <div className="aif__tbl-head-row">
                ...
                <button type="button" className="aif__add-link" onClick={openAddPartner}>
                    ...
                    {totPartners.length === 0 ? (
                      ...
                      <tr key={p.id}>
                        ...
                        <td style={{ display: "flex", gap: 6 }}>
                          <button type="button" className="aif__action-btn aif__action-btn--edit"
              ...
              <div key={key} className="aif__ipr-block">
                ...
                <div className="aif__ipr-grid">
                  ...
                  <div className="form-group" style={{ margin: 0 }}>
                    ...
                    />
                    {iprData[key].filed && !iprData[key].filingNo && (
                      <span className="aif__ipr-err">Required when Filed is checked</span>
                    )}
                  </div>
                  <div className="form-group" style={{ margin: 0 }}>
                  ...
                  <div className="form-group" style={{ margin: 0 }}>
                    ...
                    />
                    {iprData[key].granted && !iprData[key].grantNo && (
                      <span className="aif__ipr-err">Required when Granted is checked</span>
                    )}
                  </div>
                  <div className="form-group" style={{ margin: 0 }}>
        ...
        {step === 5 && (
          <div className="aif__step-body">
            <div className="aif__tbl-head-row">
              ...
              <button type="button" className="aif__add-link" onClick={() => setShowDocumentModal(true)}>
              ...
              {[...DOCUMENTATION_ITEMS, ...customDocuments].map((d) => {
                ...
                return (
                  <div key={d} className="aif__doc-row" onClick={() => toggleDoc(d)}>
                    ...
                    {attached ? (
                      <span className="aif__doc-attached" onClick={(e) => e.stopPropagation()}>
                        <span className="aif__doc-attached-name">{attached.file.name}</span>
                        <button type="button" className="aif__doc-attached-remove"
                          onClick={() => setPendingFiles((p) => p.filter((f) => f.id !== attached.id))}>
                          {Icons.close}
                        </button>
                      </span>
                    ) : (
            ...
            <input
              ...
              onChange={(e) => {
                ...
                if (!file) return;
                setPendingFiles((p) => [...p, { id: uid(), name: pendingFileName.trim() || file.name, file }]);
                setPendingFileName("");
                e.target.value = "";
              }}
            />
        ...
        {step === 6 && (
          <div className="aif__step-body">
            <div className="aif__tbl-head-row">
              ...
              <button type="button" className="aif__add-link" onClick={openAddFirm}>
