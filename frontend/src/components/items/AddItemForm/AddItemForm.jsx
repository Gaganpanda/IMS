...
import { createItemAsync, uploadImageAsync } from "../../../redux/slices/itemSlice";
import {
...
export default function AddItemForm({ onCancel, onSuccess }) {
  ...
  const savePartner = () => {
    ...
    setTotPartners((p) => [...p, { id: Date.now(), name: partnerData.name, cert: false, sampleTAC: false, latoT: false }]);
    setPartnerData({ name: "" });
  ...
  const saveFirm = () => {
    ...
    setProcurements((p) => [...p, { id: Date.now(), ...firmData }]);
          ...
          <div className="aif__step-body">
            ...
            <div>
              <div className="aif__tbl-head-row">
                ...
                <button type="button" className="aif__add-link" onClick={() => setShowPartnerModal(true)}>
                ...
                <table className="aif__tbl">
                  ...
                  <tbody>
                    {totPartners.length === 0 ? (
                      <tr><td colSpan="5" style={{ textAlign:"center", padding:"20px", color:"#94a3b8", fontSize:12.5 }}>No partners added yet</td></tr>
                    ) : totPartners.map((p) => (
                      <tr key={p.id}>
                        <td>{p.name}</td>
                        ...
                        <td>
                          <button type="button" className="aif__del-btn"
                            onClick={() => setTotPartners((x) => x.filter((t) => t.id !== p.id))}>
                            {Icons.trash}
                          </button>
                        </td>
                      </tr>
        ...
        {step === 5 && (
          <div className="aif__step-body">
            ...
            <div className="aif__doc-list">
              {[...DOCUMENTATION_ITEMS, ...customDocuments].map((item) => (
                <div key={item} className="aif__doc-row" onClick={() => toggleDoc(item)}>
                  <div className="aif__doc-left">
                    <Checkbox checked={checkedDocs.has(item)} onChange={() => {}} />
                    <span className="aif__doc-name">{item}</span>
                  </div>
                  <span className="aif__doc-icon">{Icons.file}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {step === 6 && (
          <div className="aif__step-body">
            <div className="aif__tbl-head-row">
              ...
              <button type="button" className="aif__add-link" onClick={() => setShowFirmModal(true)}>
              ...
              <table className="aif__tbl">
                ...
                <tbody>
                  {procurements.length === 0 ? (
                    <tr><td colSpan="5" style={{ textAlign:"center", padding:"20px", color:"#94a3b8", fontSize:12.5 }}>No organisations added yet</td></tr>
                  ) : procurements.map((p) => (
                    <tr key={p.id}>
                      <td>{p.firm}</td>
                      <td>{p.count}</td>
