...
const Icons = {
  ...
  warning: (
    <svg viewBox="0 0 24 24" fill="currentColor">
      <path d="M12 2 1 21h22L12 2zm0 6a1.2 1.2 0 0 1 1.2 1.2v5a1.2 1.2 0 1 1-2.4 0v-5A1.2 1.2 0 0 1 12 8zm0 10.3a1.4 1.4 0 1 1 0-2.8 1.4 1.4 0 0 1 0 2.8z" />
    </svg>
  ),
  edit: (
    <svg
...
function isOverdueLocally(f) {
  ...
  const days =
    (Date.now() - new Date(f.sampleSubmissionDate).getTime()) / 86400000;
  return days >= 7;
}
...
export default function TrialStakeholders({ value, onChange }) {
  ...
  const toggleStakeholder = (key) =>
    setStakeholders((p) =>
      p.map((s) => ({ ...s, open: s._key === key ? !s.open : s.open })),
    );

  const allOpen =
    stakeholders.length > 0 && stakeholders.every((s) => s.open);
  const toggleAll = () =>
    setStakeholders((p) => p.map((s) => ({ ...s, open: !allOpen })));
  ...
  const saveModal = () => {
    if (modalKey === "new") {
      ...
      setStakeholders((p) =>
        p.map((s) => (s._key === modalKey ? { ...s, ...modalValues } : s)),
      );
    }
    setModalKey(null);
  };
  ...
  const toggleFeedback = (skey, fkey) =>
    setOpenFeedbackMap((p) => ({
      ...p,
      [skey]: p[skey] === fkey ? null : fkey,
    }));
  ...
  const removeFeedback = (skey, fkey) =>
    setStakeholders((p) =>
      p.map((s) =>
        s._key !== skey
          ? s
          : { ...s, feedbacks: s.feedbacks.filter((f) => f._key !== fkey) },
      ),
    );
  ...
  return (
    <div className="tse">
      <div className="tse__head-row">
        ...
        <div className="tse__head-actions">
          {stakeholders.length > 1 && (
            <button
              type="button"
              className="tse__add-link tse__add-link--ghost"
              onClick={toggleAll}>
        ...
        stakeholders.map((s, sIdx) => {
          ...
          return (
            <div key={s._key} className={`tse__card${s.open ? " open" : ""}`}>
              <div
                className="tse__header"
                onClick={() => toggleStakeholder(s._key)}>
                <div className="tse__avatar">
                  {getInitials(s.stakeholderName)}
                </div>
                <div className="tse__info">
                  <div className="tse__name">
                    <span className="tse__index">{sIdx + 1}</span>
                    {s.stakeholderName || "Unnamed stakeholder"}
                    <StatusBadge
                      status={s.trialStatus || "Not Started"}
                      size="sm"
                    />
                    {overdueCount > 0 && (
                      <span
                        className="tse__warn-badge"
                        title={`${overdueCount} feedback round(s) overdue`}>
                  ...
                  <div className="tse__meta">
                    ...
                    {" · "}
                    {s.feedbacks.length} feedback round
                    {s.feedbacks.length === 1 ? "" : "s"}
                  </div>
                </div>
                <button
                  type="button"
                ...
                <div className="tse__body">
                  ...
                  <div className="tse__basic-grid">
                    <div className="tse__detail-item">
                      ...
                      <span className="tse__detail-value">
                        {s.contactPersonName || "—"}
                      </span>
                    </div>
                    <div className="tse__detail-item">
                    ...
                    <div className="tse__detail-item tse__detail-item--wide">
                  ...
                  <div className="tse__feedback-head-row">
                    <span className="tse__section-label">
                      Feedback / Trial Rounds
                    </span>
                    <button
                      type="button"
                      className="tse__add-link tse__add-link--sm"
                  ...
                  {s.feedbacks.length === 0 ? (
                    <div className="tse__empty-card tse__empty-card--sm">
                      <p>
                        No feedback rounds yet — click "Add Trial / Feedback" to
                        start one.
                      </p>
                    </div>
                  ) : (
                    <div className="tse__fb-list">
                    {s.feedbacks.map((f, idx) => {
                      ...
                      return (
                        <div
                          key={f._key}
                          className={`tse__fb-card${fOpen ? " open" : ""}${overdue ? " overdue" : ""}`}>
                          <div
                            className="tse__fb-header"
                            onClick={() => toggleFeedback(s._key, f._key)}>
                            <span className="tse__fb-badge">{idx + 1}</span>
                            <span className="tse__fb-title">
                              Feedback Round {idx + 1}
                              {f.sampleNo !== "" &&
                                f.sampleNo !== null &&
                                f.sampleNo !== undefined && (
                                  <span className="tse__fb-sample">
                                    {" "}
                                    · {f.sampleNo}{" "}
                                    {Number(f.sampleNo) === 1
                                      ? "sample"
                            ...
                            {overdue && (
                              <span
                                className="tse__warn-badge"
                                title="No feedback received for 7+ days">
                                {Icons.warning} Overdue
                              </span>
                            )}
                            <span className="tse__chevron">
                              {Icons.chevronDown}
                            </span>
                            <button
                              type="button"
                              ...
                              <div className="tse__fb-grid">
                                <div className="form-group">
                                  ...
                                  <input
                                    ...
                                    value={f.sampleNo}
                                    placeholder="No. of samples sent in this round"
                                    onChange={(e) =>
                                      updateFeedback(
                                        s._key,
                                        f._key,
      ...
      <AddRecordModal
        ...
        title={
          modalKey === "new"
            ? "Add Trial Stakeholder"
            : "Edit Trial Stakeholder"
        }
        fields={[
