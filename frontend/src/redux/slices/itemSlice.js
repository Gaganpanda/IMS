...
export const uploadVariantImageAsync = createAsyncThunk(
  ...
  async ({ id, variantId, file }, { rejectWithValue }) => {
    try {
      const formData = new FormData();
      formData.append("file", file);
      const res = await itemApi.uploadVariantImage(id, variantId, formData);
      return res.data.data;
    } catch (err) {
      return rejectWithValue(err.response?.data?.error || "Failed to upload variant image");
    }
  }
);
...
export const uploadDocumentAsync = createAsyncThunk(
  ...
  async ({ id, name, file }, { rejectWithValue }) => {
    try {
      const formData = new FormData();
      if (name) formData.append("name", name);
      formData.append("file", file);
      const res = await itemApi.uploadDocument(id, formData);
...
export const deleteDocumentAsync = createAsyncThunk(
  "items/deleteDocument",
  async ({ id, docId }, { rejectWithValue }) => {
    try {
      const res = await itemApi.deleteDocument(id, docId);
...
export const convertToVariantAsync = createAsyncThunk(
  "items/convertToVariant",
  async ({ id, ...payload }, { rejectWithValue }) => {
    try {
      const res = await itemApi.convertToVariant(id, payload);
...
export const addVariantAsync = createAsyncThunk(
  "items/addVariant",
  async ({ id, ...payload }, { rejectWithValue }) => {
    try {
      const res = await itemApi.addVariant(id, payload);
...
export const getVariantDetailAsync = createAsyncThunk(
  "items/getVariantDetail",
  async ({ id, variantId }, { rejectWithValue }) => {
    try {
      const res = await itemApi.getVariantDetail(id, variantId);
      return res.data.data;
    } catch (err) {
      const message = err.response?.data?.error || "Failed to load variant";
      return rejectWithValue(message);
    }
  }
);
...
export const updateVariantAsync = createAsyncThunk(
  "items/updateVariant",
  async ({ id, variantId, ...payload }, { rejectWithValue }) => {
    try {
      const res = await itemApi.updateVariant(id, variantId, payload);
...
export const deleteVariantAsync = createAsyncThunk(
  ...
  async ({ id, variantId }, { rejectWithValue }) => {
    try {
      const res = await itemApi.deleteVariant(id, variantId);
...
const itemSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(convertToVariantAsync.fulfilled, (state, action) => {
        if (state.selectedItem?.id === action.payload.id) state.selectedItem = action.payload;
        const idx = state.list.findIndex((i) => i.id === action.payload.id);
        if (idx !== -1) state.list[idx] = action.payload;
      })
      .addCase(addVariantAsync.fulfilled, (state, action) => {
        if (state.selectedItem?.id === action.payload.id) state.selectedItem = action.payload;
        const idx = state.list.findIndex((i) => i.id === action.payload.id);
        if (idx !== -1) state.list[idx] = action.payload;
      })
